...
p('The project passes 77 unit/control tests and 25 selected source fixtures. Tests cover financial identities, evidence integrity, qualifiers, unsupported QA, malformed model output, non-finite inputs, stale reviews and synthetic review regeneration. Browser checks confirmed supported and strict-mode answers and the Uber price-qualifier conflict. Agent-curated fixtures are not independent human ground truth; full-document semantic accuracy remains unmeasured.')
...
p('V6 citation smoke test: five remedy requests yielded four candidates, zero quotation failures and one invalid nested JSON value. Agent inspection still found irrelevant provisions, missing business-scope qualifications and truncated definitions. All candidates remain unverified. V7 requests typed values instead of model-generated JSON strings; offline tests pass, but v7 has not run live. See CITATION_SMOKE_EVALUATION.md for source findings and budget records.')
