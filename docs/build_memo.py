...
p('DealLens links three public transaction filings to evidence, source comparisons and illustrative financing sensitivities. It is a reviewable baseline, not complete legal extraction. Bounded model extraction has been evaluated live on two fields; actual human verification has not occurred. A source-matching model proposal still made an unsupported legal inference. Complex clauses remain unresolved.')
...
p('The project passes 59 unit/control tests and 16 selected source fixtures. Tests cover financial identities, evidence integrity, qualifiers, unsupported QA, malformed model output, non-finite inputs, stale reviews and synthetic review regeneration. Browser checks confirmed supported and strict-mode answers and the Uber price-qualifier conflict. Agent-curated fixtures are not independent human ground truth; full-document semantic accuracy remains unmeasured.')
...
p('Latest live test: GPT-4.1 mini returned 12 responses across two fields, yielding seven candidates (one null), three citation rejections and two empty abstentions. Four prices and Bio-Techne financing=false agree with selected evidence. Uber financing=true incorrectly infers a transaction condition from a funding guarantee; it remains unverified and excluded from supported outputs. Citation validity is not semantic correctness.')
p('Before submission, reconcile party roles, fees, award cohorts, remedies and regulatory clocks; expand complete-clause evaluation and perform actual human review. The tested fields do not establish extraction completeness. Full precision/recall and billed cost remain unmeasured; no model candidate was promoted to an approved fact.')
