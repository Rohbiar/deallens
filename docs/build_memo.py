...
p('The project passes 76 unit/control tests and 25 selected source fixtures. Tests cover financial identities, evidence integrity, qualifiers, unsupported QA, malformed model output, non-finite inputs, stale reviews and synthetic review regeneration. Browser checks confirmed supported and strict-mode answers and the Uber price-qualifier conflict. Agent-curated fixtures are not independent human ground truth; full-document semantic accuracy remains unmeasured.')
...
p('Focused v5 test: 18 responses across four fields yielded six candidates, ten quotation failures, one invalid nested JSON value and one abstention. Source inspection still found overstated remedy prohibitions, incorrect fee triggers and financing-scope errors. All candidates remain unverified. V6 now selects source-passage IDs instead of copying quotations; offline tests pass, but live effectiveness is unmeasured. See FOCUSED_RETEST.md for source assessments and budget records.')
