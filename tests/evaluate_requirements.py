...
def evaluate(root):
    ...
    for b in bundles:
        ...
        checks={
            ...
            'every_retained_citation_matches_source':all(validate_evidence(s,doc) for r in records if r.get('evidence') for s in r.get('evidence_sources',[r])),
    ...
    report={'run_id':manifest['run_id'],'audit_type':'machine artifact/control checks plus disclosed coverage; not expert approval',
            ...
            'remaining_requirements':['Complete cross-referenced complex extraction and QA coverage','Live evaluation of current prompt and retrieval changes','Verified filing-index dates and independent page inventory','Independent semantic gold set; precision and recall','Candidate technical ownership and final submission decision'],
            'documents':cases}
